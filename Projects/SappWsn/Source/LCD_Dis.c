...
#define SPI_CS1()  P1 |= 0x10    //P1_4
#define SPI_CS0()  P1 &= (~0x10) //P1_4
#define SPI_SCK1() P1 |= 0x20    //P1_5
#define SPI_SCK0() P1 &= (~0x20) //P1_5
#define SPI_SDA1() P1 |= 0x40    //P1_6	
#define SPI_SDA0() P1 &= (~0x40) //P1_6	
//#define SPI_RES1() st( P1_7 = 1;)	
//#define SPI_RES0() st( P1_7 = 0;)	
...
void PutAbyte(unsigned char x,unsigned char y,unsigned char a){
    unsigned char High;
    unsigned char Low;
    High = (a >> 4)&0xf;
    Low = (a & 0xf);
    ...
    SPI_Send(x);					//要显示字符的左上角的X轴位置
    SPI_Send(y);					//要显示字符的左上角的Y轴位置
    SPI_Send(High);					//要显示字符ASCII字符的ASCII码值
    SPI_SSSet(1);					//完成操作置SS高电平
    
    //显示ASCII字符
    SPI_SSSet(0);					//SS置低电平	
    SPI_Send(7);					//传送指令0x07
    SPI_Send(x+8);					//要显示字符的左上角的X轴位置
    SPI_Send(y);					//要显示字符的左上角的Y轴位置
    SPI_Send(Low);					//要显示字符ASCII字符的ASCII码值
    SPI_SSSet(1);					//完成操作置SS高电平
}
...
//      2007/07/19      First version
//========================================================================
void Rectangle1(unsigned char left, unsigned char top, unsigned char right,
				 unsigned char bottom, unsigned char mode)
{
	SPI_SSSet(0);
	if(mode)

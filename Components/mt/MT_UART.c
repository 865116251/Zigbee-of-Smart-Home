...
#if !defined(ZDO_COORDINATOR) && defined ( SENSORBOARD4 )//RFID 扩展板
...
#if defined ( SENSORBOARD4 )//RFID 扩展板  
...
#if (defined(PEER_COORD)||defined(PEER_ROUTER))//点对点通信或广播通信处理                
        //接收串口的数据，通过消息发送出去    
...
{     
    ...
    case 1:
        ...
        if(RfidStatus == 1){
            ...
            if((rxByte[0] != 5) || (rxByte[1] != 0)){//寻卡错误,发送错误到网关                  
                SendFindCardError();
                RfidStatus = 0;
                rxBytePtr = 0;                  
                return;  
    ...
    case 3:
        ...
        rxBytePtr++;
        if(RfidStatus == 1){
            if((rxByte[0] == 3) && (rxByte[1] == 0) && (rxByte[2] == 4) && (rxByte[3] == 0)){//寻卡正确，发送防冲突命令
                SendBuf[0] = 0x1;
                SendBuf[1] = 0x3;         
                HalUARTWrite(HAL_UART_PORT_0, &SendBuf[0], 2);//发送寻卡命令                   
                RfidStatus = 2;//等待防冲突应答状态
                rxBytePtr = 0;                  
                return;      
            }
            else{//寻卡错误,发送错误到网关                  

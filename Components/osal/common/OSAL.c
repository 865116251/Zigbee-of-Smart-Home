...
#endif		
#ifdef LIGHT				
    P0DIR |= 0x70;  // 设置P0.4,P0.5,P0.6为输出方式
    P1DIR = 0xff;  // 设置P1为输出方式							
#endif	     
#if defined(KEY) && !defined ( BROADCAST )//点对点通信、发送按键信息    
    //HalUARTWrite(HAL_UART_PORT_0, "111", 3);  
    if(f10ms){	 
        c10ms = 10;
    	f10ms = 0;  
	if(P0&0x10){//键松开			  				
	    if(fKeyValid){//发送按键					 
	        buf[0] = 1;               
                SendData(CONTROL_ENDPOINT, &buf[0],SrcShortAddr, TRANSFER_ENDPOINT,8);   
	    }  
	    KeyValue = 0;
	    fKeyValid = 0;
	} 
...
#if (defined(KEY) && defined(BROADCAST))//广播通信、发送按键信息
    if(f10ms){	 
	c10ms = 10;
    	f10ms = 0;  
	if(P0&0x10){//键松开			  				
	    if(fKeyValid){//发送按键					 
		buf[0] = 1;
             SendData(CONTROL_ENDPOINT, &buf[0],0xFFFF, TRANSFER_ENDPOINT,8);   
	    }  
	    KeyValue = 0;
	    fKeyValid = 0;
	} 

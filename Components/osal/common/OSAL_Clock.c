...
extern uint8 DataValid;
extern bool bStepEnable;//步进电机启动或停止 0--停止；1--启动
extern bool bStepDirect;//步进电机方向，0--正向；1--反向
extern uint8 bStepSpeed;
extern bool fStepDelay;
extern uint8 cStepDelay;
static uint8 bStep=0;
// 整步模式正转脉冲
uint8 pluse_table_1[] = 
{
    0x05, 0x09, 0x0a, 0x06,
...
// 整步模式反转脉冲
const uint8 pluse_table_2[] = 
{
    0x05, 0x06, 0x0a, 0x09,
};
#endif
#if defined(SENSORBOARD7)
    static uint8 LedDispLine = 0xff;
    extern uint8 MatrixLed[8];
    extern uint16 cSound;
    uint8 c10ms;
    bool f10ms;  
#endif    
    
void SensorTimeUpdate(void);
void SensorTimeUpdate(void)
{
#if defined(SENSORBOARD6)  
    if(cStepDelay>0){
    ...
    if(bStepEnable){//步进电机启动
        if(fStepDelay){
            if(bStep >=4 )
                bStep = 0;            
            if(bStepDirect)//反向
                P1 = (pluse_table_2[bStep++]<<4);
            else//正向
                P1 = (pluse_table_1[bStep++]<<4);
